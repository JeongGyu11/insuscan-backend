...
import { fetchNhPdf, searchNh } from "./adapters/nh.mjs";
import { fetchKakaoPdf, searchKakao } from "./adapters/kakao.mjs";
...
export async function searchInsurer(insurer, filters, fetchImpl = fetch) {
  ...
  if (insurer.strategy === "nh_public_xml") return searchNh(filters, fetchImpl);
  if (insurer.strategy === "kakao_public_json") return searchKakao(filters, fetchImpl);
...
async function fetchPdfBytes(document, fetchImpl) {
  ...
  if (document.insurerId === "nh") return fetchNhPdf(document, fetchImpl);
  if (document.insurerId === "kakao") return fetchKakaoPdf(document, fetchImpl);
